  ...
  threads<<Thread.new do
    until $queue.empty?
      ...
      if !url.include? "?page"
        ...
        puts ("["+ Time.now.strftime("%H:%M:%S") + "]「" + title + "」" + url).encode("GBK")
        # next if url=~/3582189|283185|3694730|3685116/
        title=title.encode("GBK")
        path ="pages/" + title.gsub(/\//, "")
        path = path.gsub(/\/|\\|'|"|\.|\:|\"/, "")
        next if path.size>100
        save_to(path, html) if url.include? ".html"
      end
    end
  end
end
...
puts ("线程数：" + threadNums.to_s).encode("GBK")
puts ("执行时间：" + ($total_time_end - $total_time_begin).to_s + "秒").encode("GBK")