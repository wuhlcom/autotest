...
module RouterPageObject
		...
		module Wan_Page
				...
				in_iframe(:src => @@ts_tag_netset_src) do |frame|
						...
						div(:net_hint, :class_name => @@ts_tag_aui_content, :frame => frame)
        ...
        def wan_save(time=60)
            ...
            unless self.wan?
				...
				def input_static_ip(ip)
						self.static_ip_element.click #增加元素点击操作以防输入失败
						self.static_ip = ip
						ip_empty       =true
						if self.static_ip.nil? || self.static_ip.empty?
								puts "input static_ip #{ip} again"
								3.times.each do
										self.static_ip_element.click #增加元素点击操作以防输入失败
										self.static_ip = ip
										ip_empty       = self.static_ip.nil? || self.static_ip.empty?
										break unless ip_empty
								end
								puts "input static_ip #{ip} failed'" if ip_empty
						end
				end

				def input_static_mask(mask)
						self.static_netmask_element.click
						self.static_netmask = mask
						mask_empty          =true
						if self.static_netmask.nil? || self.static_netmask.empty?
								puts "input static_netmask #{mask} again"
								3.times.each do
										self.static_netmask_element.click #增加元素点击操作以防输入失败
										self.static_netmask = mask
										mask_empty          = self.static_netmask.nil? || self.static_netmask.empty?
										break unless mask_empty
								end
								puts "input static_netmask #{mask} failed'" if mask_empty
						end
				end

				def input_static_gw(gateway)
						self.static_gateway_element.click
						self.static_gateway = gateway
						gw_empty            =true
						if self.static_gateway.nil? || self.static_gateway.empty?
								puts "input static_gateway #{gateway} again"
								3.times.each do
										self.static_gateway_element.click #增加元素点击操作以防输入失败
										self.static_gateway = gateway
										gw_empty            = self.static_gateway.nil? || self.static_gateway.empty?
										break unless gw_empty
								end
								puts "input static_gateway #{gateway} failed'" if gw_empty
						end
				end

				def input_static_dns(dns)
						self.static_dns_element.click
						self.static_dns = dns
						dns_empty       =true
						if self.static_dns.nil? || self.static_dns.empty?
								puts "input static_dns #{dns} again"
								3.times.each do
										self.static_dns_element.click #增加元素点击操作以防输入失败
										self.static_dns = dns
										dns_empty       = self.static_dns.nil? || self.static_dns.empty?
										break unless dns_empty
								end
								puts "input static_dns #{dns} failed'" if dns_empty
						end
				end
				...
				def static_input(ip, mask, gateway, dns)
						input_static_ip(ip)
						input_static_mask(mask)
						input_static_gw(gateway)
						input_static_dns(dns)
				end
