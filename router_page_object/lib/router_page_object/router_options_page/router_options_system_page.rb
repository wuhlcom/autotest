...
module RouterPageObject
  module Options_Page
    ...
    in_iframe(:src => @@ts_tag_advance_src) do |frame|
      ...
      checkbox(:clear_config, id: @@ts_tag_update_box, :frame => frame) #pppoe密码显示
      ##文件共享
  ...
  class OptionsPage < MainPage
    ...
    #固件升级步骤
    def update_step(url, file, time=200, flag=false)
      open_options_page(url)
      select_update #进入固件升级页面
      check_clear_config if flag #flag为true升级的同时清除配置
