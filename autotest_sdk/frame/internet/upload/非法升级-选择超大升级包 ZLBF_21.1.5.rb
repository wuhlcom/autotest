...
testcase {
    ...
    def process
        ...
        operate("3、在升级页面中，选择同方案的大的升级文件，点击升级，查看升级是否成功，PC浏览网页是否正常。") {
            @option_iframe.file_field(id: @ts_update_filename).set(@tc_upload_file_big)
            sleep @tc_wait_time
            @option_iframe.button(id: @ts_tag_update_btn).click
            uploading = @option_iframe.div(class_name: @ts_upload, text: @ts_upload_big_text)
            assert(uploading.exists?, "选择同方案的大的升级文件进行升级，没有出现提示信息")
            if @browser.div(class_name: @ts_aui_state_focus).exists? || @browser.div(class_name: @ts_aui_state).exists?
                @browser.execute_script(@ts_close_div)
            end
            sysversion_text   = @browser.span(id: @ts_tag_systemver).parent.text
