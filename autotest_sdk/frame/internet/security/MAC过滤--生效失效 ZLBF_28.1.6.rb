...
testcase {
		...
		def process
				...
				operate("3、编辑PC2规则设置为生效，保存，查看PC1、PC2能否访问PC3上面的FTP或HTTP服务器；") {
						#修改无线客户端过滤
						wifi_mac_filter_item = @advance_iframe.td(text: @wifi_mac_addr).parent
						wifi_mac_filter_item[3].links[0].click
						wifi_select_list = @advance_iframe.select_list(id: @ts_tag_fw_select1) #编辑状态select_list不使用原来的id
						wifi_select_list.select(/#{@ts_tag_filter_use}/)
						#保存mac过滤条件
						@advance_iframe.button(id: @ts_tag_save_filter1).click #编辑状态保存按钮不使用原来的id
						sleep @tc_filter_time
						wifi_mac_filter_item = @advance_iframe.td(text: @wifi_mac_addr).parent
						wifi_status          = wifi_mac_filter_item[2].text
				...
				operate("4、点击下面按钮“使所有条目失效”，查看PC1、PC2能否访问PC3上面的FTP或HTTP服务器；") {
						#使所有规则失效
						@advance_iframe.span(id: @ts_tag_allcancel).click
						sleep @tc_wait_time
						mac_filter_item = @advance_iframe.td(text: @ts_pc_mac).parent
						status          = mac_filter_item[2].text
						assert_equal(@ts_tag_filter_nouse, status, "过滤条件状态不正确")
				...
				operate("5、点击下面按钮“使所有条目生效”，查看PC1、PC2能否访问PC3上面的FTP或HTTP服务器。") {
						#使用所有生效
						@advance_iframe.span(id: @ts_tag_alladd).click
						sleep @tc_wait_time
						mac_filter_item = @advance_iframe.td(text: @ts_pc_mac).parent
						status          = mac_filter_item[2].text
						assert_equal(@ts_tag_filter_use, status, "过滤条件状态不正确")
