...
testcase {
    ...
    def process
        ...
        operate("3、在URL过滤设置界面选择白名单，添加过滤关键字www.yahoo.com,保存；") {
          ...
          # @option_iframe.div(id: "white").text   多个网址，值为：www.baidu.com\nwww.yahoo.com
          #名单中有则不添加
        ...
        operate("4、PC1,PC2是否可以访问www.sina.com.cn，www.yahoo.cn，www.baidu.com等站点；") {
          ...
          assert(!response, "#{@tc_tag_url_baidu}未添加到白名单，可以访问#{@tc_tag_url_baidu}".to_gbk)

          puts "正在访问#{@tc_tag_url_sina}站点...".to_gbk
          response = send_http_request(@tc_tag_url_sina)
          assert(!response, "#{@tc_tag_url_sina}未添加到白名单，可以访问#{@tc_tag_url_sina}".to_gbk)

          puts "正在访问#{@tc_tag_select_url}站点...".to_gbk
          response = send_http_request(@tc_tag_select_url)
          assert(response, "添加#{@tc_tag_select_url}到白名单后，不可以访问#{@tc_tag_select_url}".to_gbk)
    ...
    def clearup
        ...
        operate("3 清除白名单") {
            ...
            b_select = @option_iframe.select_list(id: @tc_tag_select) #选择白名单
            ...
            #     @option_iframe.span(text: url, class_name: @tc_intset_list_cls).click
            #     sleep 1
            #     unless @option_iframe.span(text: url, class_name: @tc_intset_list_cls).parent.link(class_name: "delete").exists?
            #       @option_iframe.span(text: url, class_name: @tc_intset_list_cls).click
            #     end
