...
testcase {
    ...
    def process
        ...
        operate("3、按提供方精确查询；") {
            rs3 = @iam_obj.usr_login_list_app_bytype(@ts_usr_name, @ts_usr_pwd, @tc_app_provider3, false, "provider")
            assert_equal(@tc_app_provider3, rs3["apps"][0]["provider"], "查询应用3失败")
            rs4 = @iam_obj.usr_login_list_app_bytype(@ts_usr_name, @ts_usr_pwd, @tc_app_provider4, false, "provider")
