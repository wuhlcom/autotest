...
testcase {
  ...
  def process
    ...
    operate("4、修改comments输入值带有空格；") {
      args = {name: @tc_appname, provider: @tc_provider, redirect_uri: @ts_app_redirect_uri, comments: @tc_app_comments_space}
      @rs  = @iam_obj.modify_apply(@admin_id, @admin_token, @client_id, args)
      assert_equal(1, @rs["result"], "修改comments输入值带有空格失败")

      #检查是否修改成功
      rs_detail = @iam_obj.apply_details(@client_id, @admin_id, @admin_token)
      assert_equal(@tc_app_comments_space, rs_detail["comments"], "修改comments简介后查询未修改成功！")
    }
  end
