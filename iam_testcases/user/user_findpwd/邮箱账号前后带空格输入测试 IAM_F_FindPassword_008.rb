...
testcase {
    ...
    def process
        ...
        operate("2、输入邮箱前后带有空格；") {
            p "输入邮箱前带有空格".encode("GBK")
            rs = @iam_obj.find_pwd_for_email(@tc_usr_email1)
            assert_equal(@tc_err_code, rs["err_code"], "输入邮箱前带有空格的邮箱账号找回密码成功或者找回失败但是错误码不正确")
            p "输入邮箱后带有空格".encode("GBK")
            rs = @iam_obj.find_pwd_for_email(@tc_usr_email2)
            assert_equal(@tc_err_code, rs["err_code"], "输入邮箱后带有空格的邮箱账号找回密码成功或者找回失败但是错误码不正确")
        }
