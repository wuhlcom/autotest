...
module TestTool
    class XML
        ...
        #获取xml中所有脚本名对应的执行结果
        #{:success => ["sdjk", "sdklgjlkd"], :failure => [{:name => "sdlkgj", :message => "eri"}, {:name => "tg", :message => "li"}]}
        def self.get_test_result(args)
            tc_result  = {}
            tc_failure = []
            tc_success = []
            args.each { |xml|
                xmlobj  = XML.new(xml)
                root_el = xmlobj.log_root[:root_el]
                root_el.elements.each { |el|
                    rs_el    = el.elements
                    ele_size = rs_el.size
                    if ele_size == 0
                        success_name = el.attributes["name"]
                        tc_success << success_name
                    else
                        rs_el.each do |item|
                            failure = {}
                            fail_name         = item.attributes["type"]
                            fail_message      = item.attributes["message"]
                            failure[:name]    = fail_name
...
if __FILE__==$0
    ...
    # xml_path    = Dir.glob("#{report_path}/reports/#{report_dir}/*.xml")
    # p rs = TestTool::XML.get_test_result(xml_path)
    # p rs[:failure].size
    # p rs[:success].size
