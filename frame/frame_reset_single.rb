...
#Minitest::Reporters.use! [Minitest::Reporters::DefaultReporter.new(:color => true), Minitest::Reporters::JUnitReporter.new("reports", false)]
module Frame
		class WebTestReset<WebTestNameSpace
				current_path      = File.dirname(File.expand_path(__FILE__))
				@@report_path_reset = current_path+"/reports/#{$report_date}/reset"
				#stdio("#{@@report_path_res}/reset.log")
				frame             = File.new("frame_all.xml")
				doc               = REXML::Document.new frame

				@ts_el=""
				doc.root.elements.each("//TestSuite") do |ts|
						if ts.elements["name"].text=~/reset/
								@ts_el=ts
						end
				end
				...
				tcs_el.elements.each("TestCase") { |tc|
						...
						next if tc_name !~ /DMZ-开启 ZLBF_18.1.1/
						...
						if File.exist?(tc_file) && tc.attributes["auto"]=~/y/i
								File.open(tc_file, "r") { |tc|
										...
										define_method "test_#{tc_name}" do
												report_fpath = @@report_path_reset+"/#{tc_name}.log"
												...
												begin
														...
														puts "======================================================="
														report_image1 = @@report_path_reset+"/images/#{tc_name}_progress.png"
														...
														begin
																...
																puts "======================================================="
																report_image2 = @@report_path_reset+"/images/#{tc_name}_clearup.png"
