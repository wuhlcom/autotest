...
testcase {
		...
		def prepare
				@tc_lan_ip1 ="1.100.100.1"
				@tc_lan_pre1= @tc_lan_ip1.slice(/\d+\.\d+\.\d+/)
				@tc_lan_ip2 ="223.100.100.1"
		...
		def process
				...
				operate("2、分别更改DHCP服务地址为1.100.100.1和223.100.100.1") {
				...
				operate("4、更改DHCP地址为 192.168.100.1") {
						puts "修改LAN DHCP服务IP为#{@ts_default_ip}".to_gbk
						@tc_default_pre= @ts_default_ip.slice(/\d+\.\d+\.\d+/)
				...
				operate("5、保存，客户端是否自动获取IP地址为更改网段的IP地址、子网掩码、网关、DNS服务器信息；") {
						rs     = ipconfig(@ts_ipconf_all)
						pc_ip  = rs[@ts_nicname][:ip][0]
						pc_gw  = rs[@ts_nicname][:gateway][0]
						pc_dns = rs[@ts_nicname][:dns_server]
						...
						puts "PC 网关为:#{pc_gw}".to_gbk
						puts "PC DNS为:#{pc_dns.join(",")}".to_gbk
						dns_flag = pc_dns.include?(@ts_default_ip)
						assert_match(/#{@tc_default_pre}/, pc_ip, "更改LAN DHCP服务IP后PC未获取到新网段IP")
						assert_equal(@ts_default_ip, pc_gw, "更改LAN DHCP服务IP后PC网关未更新")
						assert(dns_flag, "更改LAN DHCP服务IP后PC DNS未更新")
				}
