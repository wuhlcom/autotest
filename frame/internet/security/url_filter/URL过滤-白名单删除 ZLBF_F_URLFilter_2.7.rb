...
testcase {
    ...
    def process
        ...
        operate("2、进入到URL过滤设置页面；选择白名单，增加过滤项：www.sina.com.cn，保存生效") {
            ...
            assert(response, "URL过滤失败，#{@tc_tag_url_sina}已添加到白名单，PC1不能访问外网")
            response_pc2 = @tc_dumpcap_pc2.send_http_request(@tc_tag_url_sina)
            assert(response_pc2, "URL过滤失败，#{@tc_tag_url_sina}已添加到白名单，PC2不能访问外网")
        }
        ...
        operate("4、在PC1和PC2上是否可以访问www.sina.com。") {
            response = send_http_request(@tc_tag_url_sina)
            refute(response, "URL过滤失败，#{@tc_tag_url_sina}未添加到白名单，PC1仍能访问外网")
            response_pc2 = @tc_dumpcap_pc2.send_http_request(@tc_tag_url_sina)
            refute(response_pc2, "URL过滤失败，#{@tc_tag_url_sina}未添加到白名单，PC2仍能访问外网")
        }
