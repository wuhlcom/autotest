...
testcase {
    ...
    def process
        ...
        operate("1、BAS开启抓包；") {
            ...
            assert_equal(@tc_pppoe_chap, rs["authentication"], "修改认证方式失败")
        }

        operate("2、设置DUT的WAN拨号方式为PPPoE，DNS为自动获取方式，BAS认证强制设置为CHAP-MD5，并填写正确的拨号用户名和密码，提交；") {
            @wan_page = RouterPageObject::WanPage.new(@browser)
            @wan_page.set_pppoe(@ts_pppoe_usr, @ts_pppoe_pw, @browser.url)
        }

        operate("3、抓包确认在PPP LCP过程中，BAS与DUT协商是否采用CHAP-MD5认证，拨号是否成功，查看WAN连接，IP，路由，DNS等信息统计页面显示信息是否正确；") {
