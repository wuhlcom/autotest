...
testcase {
    ...
    def prepare
        @tc_upload_file_txt = Dir.glob(@ts_upload_directory+"/*").find { |file| file=~/txt/i }
        p "脚本要使用的升级文件是：#{@tc_upload_file_txt}".to_gbk
    ...
    def process
        ...
        operate("2、进入升级页面；") {
            ...
            @options_page.set_update_file(@tc_upload_file_txt)
            sleep 1
