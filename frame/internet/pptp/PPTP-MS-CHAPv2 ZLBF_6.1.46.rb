...
#
testcase {
    attr = {"id" => "ZLBF_6.1.46", "level" => "P2", "auto" => "n"}

    def prepare
        @tc_dumpcap_server = DRbObject.new_with_uri(@ts_drb_server2)
        @tc_pptp_mschap2   = "mschap2"
        @tc_pptp_usr_err   = "err"
        @tc_pptp_pw_err    = "err"
    end
    ...
    def process
        ...
        operate("1、BAS开启抓包；") {
            #设置ppptp服务的加密为mschap2
            @tc_dumpcap_server.init_routeros_obj(@ts_pptp_server_ip)
            @tc_dumpcap_server.routeros_send_cmd(@ts_pptp_mschap2_set)
            rs = @tc_dumpcap_server.pptp_srv_pri(@pptp_pri)
            p "修改服务器PPTP认证方式为:#{rs["authentication"]}".to_gbk
            assert_equal(@tc_pptp_mschap2, rs["authentication"], "修改认证方式失败")
        }

        operate("2、设置DUT的WAN拨号方式为PPTP，DNS为自动获取方式，BAS认证强制设置为MS-CHAPv2，设置PPTP服务器的IP地址为192.168.25.9，网关地址为192.168.25.9,IP地址设置与服务器同一网段，例如：192.168.25.100，mask填写为255.255.255.0，并填写正确的拨号用户名和密码，提交并保存设置；") {
            @options_page = RouterPageObject::OptionsPage.new(@browser)
            @sys_page     = RouterPageObject::SystatusPage.new(@browser)
            @options_page.set_pptp(@ts_pptp_server_ip, @ts_pptp_usr, @ts_pptp_pw, @browser.url)
        }

        operate("3、抓包确认在PPP LCP过程中，BAS与DUT协商是否采用MS-CHAPv2认证(协议码：0xc223,Algorithm:0x80)，拨号是否成功，查看WAN连接，IP，路由，DNS等信息统计页面显示信息是否正确；") {
