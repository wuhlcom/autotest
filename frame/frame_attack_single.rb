...
module Frame
		class WebTestAttack<WebTestNameSpace
				current_path         = File.dirname(File.expand_path(__FILE__))
				@@report_path_backup = current_path+"/reports/#{$report_date}/attack"
				frame                = File.new("frame_all.xml")
				doc                  = REXML::Document.new frame

				@ts_el=""
				doc.root.elements.each("//TestSuite") do |ts|
						if ts.elements["name"].text=~/attack/
								@ts_el=ts
						end
				end
				...
				tc_el  = tcs_el.elements.each("TestCase") { |tc|
						...
						next if tc_name !~/ZLBF_Attack_1.1.10/
