...
#
testcase {
		attr = {"id" => "ZLBF_21.1.56", "level" => "P1", "auto" => "n"}
		...
		def process
				...
				operate("2、再次更改新的密码，使用新的密码重新登陆web是否成功；") {
						...
						assert(rs, "修改账户失败")
						@account_page.login_with(@tc_new_usr, @tc_new_pw, @browser.url)
						rs = @account_page.lan?
						assert(rs, "新账户登录失败")

						puts "修改为默认账户!".to_gbk
						@account_page.modify_account(@browser.url, @ts_default_usr, @ts_default_pw) #修改账户为默认
						@account_page.login_with(@ts_default_usr, @ts_default_pw, @browser.url)
						rs = @account_page.lan?
						assert(rs, "使用默认账户登录失败")
				}
