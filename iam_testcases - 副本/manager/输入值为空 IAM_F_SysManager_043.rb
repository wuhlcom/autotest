...
testcase {
    ...
    def process
        ...
        operate("3、新增一个管理员，comments为空；") {
            #如果管理员已经存在则先删除
            @iam_obj.del_manager(@tc_man_name)
            puts "新增管理员:‘#{@tc_man_name}’，备注为空'".encode("GBK")
            rs = @iam_obj.manager_add(@tc_man_name, @tc_nickname, @tc_passwd, @tc_rcode, @tc_comment)
            puts "RESULT MSG:#{rs['msg']}".encode("GBK")
            assert_equal(@ts_add_rs, rs["result"], "新增管理员:‘#{@tc_man_name}’，备注为空失败!")
            assert_equal(@ts_add_msg, rs["msg"], "新增管理员:‘#{@tc_man_name}’，备注为空失败!")
        }
