...
testcase {
  ...
  def process
    ...
    operate("3、修改nickname信息；") {
      puts "昵称#{@tc_args["nickname"]}的长度为#{@tc_args["nickname"].size}".to_gbk
      rs = @iam_obj.edit_mana_info(@tc_args, @tc_man_name, @tc_passwd)
      puts "RESULT err_msg:#{rs['err_msg']}".encode("GBK")
      puts "RESULT err_code:#{rs['err_code']}".encode("GBK")
      puts "RESULT err_desc:#{rs['err_desc']}".encode("GBK")
      assert_equal(@ts_err_nickformat_code, rs["err_code"], "昵称长度为#{@tc_args["nickname"].size}返回code错误!")
      assert_equal(@ts_err_nickformat, rs["err_msg"], "昵称长度为#{@tc_args["nickname"].size}返回msg错误!")
      assert_equal(@ts_err_nickformat_desc, rs["err_desc"], "昵称长度为#{@tc_args["nickname"].size}desc!")
    }
