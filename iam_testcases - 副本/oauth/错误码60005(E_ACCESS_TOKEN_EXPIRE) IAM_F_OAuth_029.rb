...
testcase {
  ...
  def process
    ...
    operate("7、code有效期内获取认证access_token值；") {
      ...
      refute_empty(@acc_token, "code有效期内获取认证access_token值失败!")
    }
    operate("8、获取用户信息") {
      rs = @iam_obj.usr_oauth_info(@acc_token)
      assert_equal(@usr_login_id, rs["id"], "获取用户信息失败!")
      assert_equal(@tc_phone, rs["account"], "获取用户信息失败!")
    }

    operate("9、access_token超时") {
      tip = "等待一小时,access_token超时"
      puts "sleeping #{@tc_ahour/60} minutes..."
      sleep @tc_ahour
      rs = @iam_obj.usr_oauth_info(@acc_token)
      puts "RESULT err_msg:'#{rs['err_msg']}'".encode("GBK")
      puts "RESULT err_code:'#{rs['err_code']}'".encode("GBK")
      puts "RESULT err_desc:'#{rs['err_desc']}'".encode("GBK")
      assert_equal(@ts_err_oautokenex_msg, rs["err_msg"], "#{tip}返回错误消息不正确!")
      assert_equal(@ts_err_oautokenex_code, rs["err_code"], "#{tip}返回错误code不正确!")
      assert_equal(@ts_err_oautokenex_desc, rs["err_desc"], "#{tip}返回错误desc不正确!")
    }
