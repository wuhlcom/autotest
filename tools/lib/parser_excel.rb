...
module TestTool
    class Excel
        ...
        #获取用例名，ID，级别,无过滤条件
        def get_name_id_level_no_condition(args)
            #name列
        ...
        #生成测试用例名字，级别，ID，步骤等Hash组 create_tcname_no_condtion
        def create_tcname_no_condtion(args)
            #ID列
            args[:id_col]      = "D" if args[:id_col].nil?
            id_col             = args[:id_col]
            #起始行号
...
# 		file_name="基线用例_V1.0.7_2015.10.29_带三期.xls"
    ...
    begin
        ...
        excelobj.create_tcname_no_condtion(args)
