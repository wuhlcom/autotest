...
testcase {
		...
		attr = {"id" => "ZLBF_Attack_1.1.11", "level" => "P1", "auto" => "n"}
		...
		def process
				...
				operate("4 发送报文前查看进程是否存在") {
						...
						rs = router_attack_ps
						assert(rs, "发送UDP random报文前路由器进程异常")
				}

				operate("5 发送UDP random报文") {
						...
						rs      = pktobj.send_udp("%%%%%", "%%%%%", @tc_count, @tc_delay)
						assert(rs.kind_of?(Hash), "报文发送失败")
				}

				operate("6 发送UDP random报文后查看进程") {
						puts "发送UDP random报文后查看udhcpc、udhcpd、lighttpd、dnsmasq、ntpd进程".to_gbk
						rs = router_attack_ps
						assert(rs, "发送UDP random报文后路由器进程异常")
				}

				operate("7 发送UDP random报文后验证业务") {
						rs1 = ping(@ts_default_ip)
						assert(rs1, '发送UDP random报文后路由器无法登录')
						rs2 = ping(@ts_web)
						assert(rs2, '发送UDP random报文后无法连接外网')
						puts "发送UDP random报文后查看路由器WEB是否可以登录".to_gbk
						@wan_page.refresh
						rs3 = @wan_page.wan?
						assert(rs3, '发送UDP random报文后无法打开WEB界面')
				}
