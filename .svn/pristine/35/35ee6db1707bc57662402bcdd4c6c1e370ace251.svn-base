...
module TestTool
    class CreateExcel
        ...
        def xml_to_excel(args)
            ...
            # all_arr  = xml_obj.get_testcase.sort #按测试套顺序
            all_arr = xml_obj.get_testcase_module.sort #按模块顺序
            params   = {
                :range => "A1:E1",
                :value => ["功能模块".encode("GBK"),
                           "测试用例".encode("GBK"),
                           "测试套".encode("GBK"),
                           "是否执行".encode("GBK"),
                           "备注".encode("GBK")]
            ...
            all_arr.each do |key, value|
                value.each do |item|
                    rangeobj("A#{line}:D#{line}")
                    # set_range([item[:tc_module], item[:tc_name], key.to_s, "N"]) #按测试套顺序
                    set_range([key.to_s, item[:tc_name], item[:ts_name], "N"]) #按模块顺序
                    line+=1
                end
            end
        end
