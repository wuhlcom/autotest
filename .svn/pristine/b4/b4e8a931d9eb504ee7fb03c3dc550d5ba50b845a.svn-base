...
testcase {
		...
		def process
				...
				operate("4、修改LAN口IP地址为192.168.2.1，修改完成后，查看规则1的地址段是否自动变更为192.168.2.2到192.168.2.10，其他数据不变") {
						...
						@options_page.select_traffic_ctl(@browser.url)
						ip_seg = @options_page.ip_segment0
						puts "修改LAN IP地址段后流量控制IP地址段为#{ip_seg}"
						tc_ip_seg = @tc_lan_ip1.sub(/\d$/, "")
						puts "IP PRE:#{tc_ip_seg}"
						assert_equal(tc_ip_seg, ip_seg, "修改LAN IP后带宽控制IP网段未自动改变")
				}
