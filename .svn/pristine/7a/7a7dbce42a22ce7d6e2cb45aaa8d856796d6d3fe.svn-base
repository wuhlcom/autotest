...
testcase {
    ...
    def process
        ...
        operate("4、在PC1和PC2上是否可以访问www.sina.com。") {
            p "2.PC1对外网#{@url}发送http请求".to_gbk
            response = send_http_request(@url)
            assert(response, "在过滤规则中没有#{@url}时，PC1不能访问外网#{@url}".to_gbk)
            p "3.PC2对外网#{@url}发送http请求".to_gbk
            response_pc2 = @tc_dumpcap_pc2.send_http_request(@url)
            assert(response_pc2, "在过滤规则中没有#{@url}时，PC2不能访问外网#{@url}".to_gbk)
        }
