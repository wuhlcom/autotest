...
testcase {
		...
		attr = {"id" => "ZLBF_Attack_1.1.4", "level" => "P1", "auto" => "n"}
		...
		def process
				...
				operate("4 发送报文前查看进程是否存在") {
						...
						rs = router_attack_ps
						assert(rs, "发送ICMP Flood报文前路由器进程异常")
				}
				...
				operate("5 发送ICMP Flood报文") {
						...
						rs      = pktobj.send_icmp(@tc_count)
						assert(rs.kind_of?(Hash), "报文发送失败")
				}

				operate("6 发送ICMP Flood报文后查看进程") {
						puts "查看udhcpc、udhcpd、lighttpd、dnsmasq、ntpd进程".to_gbk
						rs = router_attack_ps
						assert(rs, "发送ICMP Flood报文后路由器进程异常")
				}

				operate("7 发送ICMP Flood报文后验证业务") {
						rs1 = ping(@ts_default_ip)
						assert(rs1, '发送ICMP Flood报文后路由器无法登录')
						rs2 = ping(@ts_web)
						assert(rs2, '发送ICMP Flood报文后无法连接外网')
				}
