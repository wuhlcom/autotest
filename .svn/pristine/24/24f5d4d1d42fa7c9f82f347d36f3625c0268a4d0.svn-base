testsuite {
    ...
    def setup
        ...
        @tc_name = self.to_s.encode("UTF-8")
        if @tc_name =~ /桥|无线wan|bridge/i
            p "执行无线WAN脚本前先将交换机连接路由器WAN口的端口关闭！".encode("GBK")
            wan_mac      = @ts_wan_mac.split(":")
            mac_addr_lsw = (wan_mac[0]+wan_mac[1]+"."+wan_mac[2]+wan_mac[3]+"."+wan_mac[4]+wan_mac[5]).downcase
            lsw_telnet_obj
            @lsw_port = get_port(mac_addr_lsw) #通过wan口mac地址获取交换机上对应的端口号
            shutdown_port(@lsw_port)
        end
    end
    ...
    def teardown
        @browser.close
        if @tc_name =~ /桥|无线wan|bridge/i
            p "执行无线WAN脚本后将交换机连接路由器WAN口的端口打开！".encode("GBK")
            unless @lsw_port.nil?
                lsw_telnet_obj
                no_shutdown_port(@lsw_port)
            end
        end
    end
