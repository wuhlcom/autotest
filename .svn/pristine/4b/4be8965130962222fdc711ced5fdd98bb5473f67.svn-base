...
testcase {
    ...
    def process
        ...
        operate("2、配置被测AP为通过桥接方式上网。PC1上网成功") {
            ...
            assert_match(@ip_regxp, wan_addr, 'Bridge获取ip地址失败！')
            assert_match(/#{@ts_wan_mode_bridge}/, wan_type, '接入类型错误！')
            assert_match(@ip_regxp, mask, 'Bridge获取ip地址掩码失败！')
            assert_match(@ip_regxp, gateway_addr, 'Bridge获取网关ip地址失败！')
            assert_match(@ip_regxp, dns_addr, 'Bridge获取dns ip地址失败！')
            p "判断是否上网成功".to_gbk
