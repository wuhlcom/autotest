...
#
testcase {
    attr = {"id" => "ZLBF_6.1.43", "level" => "P2", "auto" => "n"}
    ...
    def process
        ...
        operate("1、BAS开启抓包；") {
            ...
            @tc_dumpcap_server.routeros_send_cmd(@ts_pptp_pap_set)
            rs = @tc_dumpcap_server.pptp_srv_pri(@pptp_pri)
            p "修改服务器PPTP认证方式为:#{rs["authentication"]}".to_gbk
            assert_equal(@tc_pptp_pap, rs["authentication"], "修改认证方式失败")
        }

        operate("2、设置DUT的WAN拨号方式为PPTP，DNS为自动获取方式，BAS认证强制设置为PAP，设置PPTP服务器的IP地址为192.168.25.9，网关地址为192.168.25.9,IP地址设置与服务器同一网段，例如：192.168.25.100，mask填写为255.255.255.0，并填写正确的拨号用户名和密码，提交并保存设置；") {
        ...
        operate("3、抓包确认在PPP LCP过程中，BAS与DUT协商是否采用PAP认证(协议码：0xc023)，拨号是否成功，查看WAN连接，IP，路由，DNS等信息统计页面显示信息是否正确；") {
