...
module HtmlTag
		...
		module WinCmd
				...
				def self.ping(ip, count=5)
						...
						str  = "无法访问目标网"
						if !loss.nil?&&loss.to_i<60&&rs_utf8 !~/#{str}/
								true
						else
								false
						end
				...
				def nic_disabled(ip, nicname, args_address, args_dhcp, count)
						result=true
						rs_ip = ""
				...
				def nic_enabled(ip, rs, nicname, args_address, args_dhcp, count)
						result=true
						rs_ip = ""
				...
				def ping_recover(ip, nicname="dut", count=5)
						...
						#如果网卡被初始状态为禁用
						if rs[:admin_state]==DISABLED
								result = nic_disabled(ip, nicname, args_address, args_dhcp, count)
						else
								result = nic_enabled(ip, rs, nicname, args_address, args_dhcp, count)
