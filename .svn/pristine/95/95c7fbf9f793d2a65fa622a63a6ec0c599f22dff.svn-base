...
testcase {
		...
		def process
				...
				operate("1、PC登录到路由器，点击页面上设置向导按钮") {
						...
						#查看向导是否跳到WIFI设置
						wifiset_wz    = @wifi_page.wifiset_wz_element.visible?
						next_step = @wifi_page.wifiset_wz_map?
						assert(wifiset_wz, "未出现设置WIFI设置向导")
						assert(next_step, "未出现跳过按钮")
				}

				operate("2、进入向导设置完成步骤，点击开始使用完成向导设置") {
						#点击WIFI设置向导的"跳过"
						@wifi_page.wifiset_wz_map_element.area_element.click
						#查看向导是否跳到WIFI设置
						wz    = @wifi_page.wz_finish_element.visible?
						next_step = @wifi_page.wz_finish_map?
						assert(wz, "未出现设置完成向导")
						assert(next_step, "未出现开始使用按钮")

						#点击开始使用完成向导
						@wifi_page.wz_finish_map_element.area_element.click
						#查看向导是否关闭
						wz_finish    = @wifi_page.wz_finish_element.visible?
						refute(wz_finish, "向导未关闭")
				}
