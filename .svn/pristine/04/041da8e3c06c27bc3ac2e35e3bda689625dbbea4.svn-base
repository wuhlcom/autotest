...
testcase {
		...
		def process
				...
				operate("2、结束IP输入x.x.x.254,带宽为100,其它值为默认值保存") {
						tc_qos_ip      = "254"
						puts "输入结束IP地址为:#{tc_qos_ip}".encode("GBK")
						@options_page.bw_ip_max0=tc_qos_ip
				}

				operate("3、起始IP地址输入有空格如\"2 5\",\" 25\",\"25 \"") {
						tc_qos_err_ip1 = "2 5"
						tc_qos_err_ip2 = " 25"
						tc_qos_err_ip3 = "25 "

						puts "输入起始IP地址为：#{tc_qos_err_ip1}".encode("GBK")
						@options_page.bw_ip_min0=tc_qos_err_ip1
