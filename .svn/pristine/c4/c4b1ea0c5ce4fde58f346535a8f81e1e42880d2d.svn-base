...
testcase {
		...
		def prepare
				...
				@tc_bandwidth_limit     = 1000
				#bps
				@tc_bandwidth_total_bps = 10240000
				@tc_bandwidth_limit_bps = 1024000
		...
		def process
				...
				operate("2、配置满配置规则(8条)，起始IP和结束IP相同，从2开始递直到满配置，模式都为限制最大带宽为100kbps，规则为启用状态") {
						#打开高级设置
						...
						@options_page.set_client_bw(1, @tc_pc_ip.split(".").last, @tc_pc_ip.split(".").last, @ts_tag_bandlimit, @tc_bandwidth_limit)
						#规则2
						@options_page.add_item
						@options_page.set_client_bw(2, @tc_wifi_ip.split(".").last, @tc_wifi_ip.split(".").last, @ts_tag_bandlimit, @tc_bandwidth_limit)
						#规则3
						@options_page.add_item
						@options_page.set_client_bw(3, (@tc_wifi_ip.split(".").last.to_i+3).to_s, (@tc_wifi_ip.split(".").last.to_i+3).to_s, @ts_tag_bandlimit, @tc_bandwidth_limit)
						#规则4
						@options_page.add_item
						@options_page.set_client_bw(4, (@tc_wifi_ip.split(".").last.to_i+4).to_s, (@tc_wifi_ip.split(".").last.to_i+4).to_s, @ts_tag_bandlimit, @tc_bandwidth_limit)
						#规则5
						@options_page.add_item
						@options_page.set_client_bw(5, (@tc_wifi_ip.split(".").last.to_i+5).to_s, (@tc_wifi_ip.split(".").last.to_i+5).to_s, @ts_tag_bandlimit, @tc_bandwidth_limit)
						#规则6
						@options_page.add_item
						@options_page.set_client_bw(6, (@tc_wifi_ip.split(".").last.to_i+6).to_s, (@tc_wifi_ip.split(".").last.to_i+6).to_s, @ts_tag_bandlimit, @tc_bandwidth_limit)
						#规则7
						@options_page.add_item
						@options_page.set_client_bw(7, (@tc_wifi_ip.split(".").last.to_i+7).to_s, (@tc_wifi_ip.split(".").last.to_i+7).to_s, @ts_tag_bandlimit, @tc_bandwidth_limit)
						#规则8
						@options_page.add_item
						@options_page.set_client_bw(8, (@tc_wifi_ip.split(".").last.to_i+8).to_s, (@tc_wifi_ip.split(".").last.to_i+8).to_s, @ts_tag_bandlimit, @tc_bandwidth_limit)
						@options_page.save_traffic #保存
				}

				operate("3、AP下接PC的IP地址分别设置为规则对应的IP，并都进行FTP下载，是否都受限为100kbps") {
