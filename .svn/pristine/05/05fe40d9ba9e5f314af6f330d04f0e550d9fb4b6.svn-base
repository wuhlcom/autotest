...
testcase {
		...
		def process
				...
				operate("3、打开文件共享，进入英文文件夹下载英文文件") {
						...
						udisk_dir_status = @fileshare_page.udisk_first_dir_en?
						assert(udisk_dir_status, "路由器未显示目录:#{@tc_share_dir}")

						#查看SD卡中的文件
						puts "查看文件:#{@tc_download_file}是否存在".to_gbk
						udisk_file_status = @fileshare_page.udisk_first_file4?
						assert(udisk_file_status, "路由器未显示测试文件:#{@tc_download_file}")
						file_size = @fileshare_page.get_udisk_first_file4_size
						...
						@fileshare_page.udisk_first_file4 #下载文件
