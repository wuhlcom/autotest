...
testcase {
		...
		def process
				# 1、进入DUT 带宽控制页面；
				# 2、勾选“开启IP带宽控制”选项框，设置申请带宽为10000kbps；
				# 3、添加一条流量控制规则，如IP地址范围输入192.168.1.2，运行模式设置受限最大上行带宽为1000kbps，
				# 4、设置该规则为开启状态，保存；
				# 5、在IP地址为192.168.1.2的PC上运行FTP下载
				# 6、重启DUT
				# 7、重启后查看配置并进行FTP下载
				...
				operate("5、在IP地址为#{@ts_pc_ip}的PC上进行FTP下载") {
