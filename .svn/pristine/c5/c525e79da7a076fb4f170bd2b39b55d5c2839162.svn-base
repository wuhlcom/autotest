...
module Frame
		class WebTestApMode<WebTestNameSpace
				current_path      = File.dirname(File.expand_path(__FILE__))
				@@report_path_apmode = current_path+"/reports/#{$report_date}/apmode"
				#stdio("#{@@report_path_apmode}/internet.log")
				...
				doc.root.elements.each("//TestSuite") do |ts|
						if ts.elements["name"].text=~/apmode/
								@ts_el=ts
						end
				end
				...
				tc_el  = tcs_el.elements.each("TestCase") { |tc|
						...
						next if tc_name !~ /AP模式无线上网 ZLBF_ApMode_1.1.5/i #中继的暂不执行 2015-10-19
						...
						if File.exist?(tc_file) && tc.attributes["auto"]=~/y/i
								File.open(tc_file, "r") { |tc|
										...
										define_method "test_#{tc_name}" do
												report_fpath = @@report_path_apmode+"/#{tc_name}.log"
												...
												begin
														...
														puts "======================================================="
														report_image1 = @@report_path_apmode+"/images/#{tc_name}_progress.png"
														...
														begin
																...
																puts "======================================================="
																report_image2 = @@report_path_apmode+"/images/#{tc_name}_clearup.png"
