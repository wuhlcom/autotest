...
#
testcase {
    attr = {"id" => "ZLBF_4.1.12", "level" => "P2", "auto" => "n"}
    ...
    def process
        ...
        operate("1、配置外网设置为桥接模式，扫描一个可以上网的上行AP进行连接，输入正确的密码连接成功；") {
        ...
        operate("3、扫描一个上行AP进行连接，输入错误的密码进行连接，使连接不成功；") {
            ...
            until ssid_flag == true
                ...
                begin
                    ...
                    @wan_iframe.span(id: @ts_wireless_id).click #无线连接
                    @wan_iframe.label(id: @ts_bridge_id).click #选择桥接模式
                    @wan_iframe.button(id: @ts_search_net).wait_until_present(@ts_search_ssid_wait_time)
                    @wan_iframe.button(id: @ts_search_net).click #点击扫描网络
                end
                sleep @tc_wait_time
                select_click = @wan_iframe.select_list(id: @ts_ssid_list)
