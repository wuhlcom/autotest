...
testcase {
  ...
  def process
    ...
    operate("8、code超时后获取认证access_token值；") {
      tip= "等待#{@tc_timeout}sencods code超时"
      puts tip.to_gbk
      sleep @tc_timeout
      rs = @iam_obj.usr_oauth_token(@usr_code, @app_id, @app_secret)
      puts "RESULT err_msg:'#{rs['error']}'".encode("GBK")
      puts "RESULT err_code:'#{rs['error_code']}'".encode("GBK")
      assert_equal(@ts_err_oauth_code, rs['error_code'], "#{tip}返回错误消息不正确!")
      assert_equal(@ts_err_oauth_msg, rs['error'], "#{tip}返回错误code不正确!")
    }
