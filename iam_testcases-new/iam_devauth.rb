...
module IAM
  class IamDevauth< IamTestNameSpace
    current_path          = File.dirname(File.expand_path(__FILE__))
    @@report_path_devauth = current_path+"/reports/#{$report_date}/manager"
    xml                   = File.new("iam_all.xml")
    doc                   = REXML::Document.new(xml)
    @ts_el=""
    doc.root.elements.each("//TestSuite") do |ts|
      if ts.elements["name"].text=~/devauth/
        @ts_el=ts
      end
    end
      ...
      if File.exist?(tc_file) && tc.attributes["auto"]=~/y/i
        File.open(tc_file, "r") { |tc|
          ...
          define_method "test_#{tc_name}" do
            report_fpath = @@report_path_devauth+"/#{tc_name}.log"
