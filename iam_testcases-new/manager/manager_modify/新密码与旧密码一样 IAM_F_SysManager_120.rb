...
testcase {
  ...
  def prepare
    @tc_man_name = "autotest_003@zhilutec.com"
  ...
  def process
    ...
    operate("3、修改为新密码和旧密码一样；") {
      ...
      assert_equal(@ts_err_pwsame_code, rs_mod["err_code"], "#{tip}提示code错误!")
      assert_equal(@ts_err_pwsame_msg, rs_mod["err_msg"], "#{tip}提示msg错误!")
      assert_equal(@ts_err_pwsame_desc, rs_mod["err_desc"], "#{tip}提示desc错误!")
    }
